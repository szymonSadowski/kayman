...
const SUPPORTED_PROVIDERS = ['openai', 'anthropic', 'google', 'ollama'] as const
...
export function createProviderModel(config: Config): LanguageModel {
  ...
  switch (aiProvider) {
    ...
    case 'ollama': {
      const baseURL = (config.aiBaseUrl ?? 'http://localhost:11434') + '/api'
      return createOllama({ baseURL })(aiModel) as unknown as LanguageModel
    }
    default:
